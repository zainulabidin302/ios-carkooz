...
class MyCarsTableViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let navController = segue.destination as? UINavigationController {
            ...
            if let recieverVC = navController.topViewController as? CarViewDetailsViewController {
    ...
    @IBAction func getData() {
        ...
        let task = session.dataTask(with: request) { (data, response, error) in
            ...
            guard let json = (try? JSONSerialization.jsonObject(with: content, options: [])) as? [[String: Any]] else {
                ...
                AppDelegate.showToast(message: "No record found", duration: 4, controller: self)
                return
            }
            if (json.count == 1) {
                guard let error = json[0]["model"] else {
